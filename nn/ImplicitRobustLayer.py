import torch
from nn import ImplicitFunctionTriu, ImplicitLayer, ImplicitPerturbation
from torch import nn

from utils import transpose


class ImplicitRobustLayer(ImplicitLayer):
    def __init__(self, n, m, p, q, f=ImplicitFunctionTriu, X0=None, no_D=False):
        super(ImplicitRobustLayer, self).__init__(n, m, p, q, f, X0, no_D=no_D)
        self.perturb = ImplicitPerturbation()


    def forward(self, U, sigma_u, proxy=False):
        Y = super(ImplicitRobustLayer, self).forward(U)
        sigma = self.perturb(self.A, self.B, self.C, self.D, sigma_u, proxy=proxy)
        return Y, sigma#torch.zeros_like(sigma)#sigma

class ImplicitRobustLayerRank1FT(ImplicitRobustLayer):
...
class ImplicitRobustLayerTriuFT(ImplicitRobustLayer):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
