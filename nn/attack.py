...
from nn import ImplicitLayer, ImplicitRobustLayer, ImplicitRobustLayerRank1FT
...
def fgsm_test(model, device, test_loader, epsilon, attack_num=100, do_print=True):
    # Accuracy counter
    m = (lambda x: model(x, 0.0)[0]) if isinstance(model, (ImplicitRobustLayer, ImplicitRobustLayerRank1FT)) else model
    correct = 0
    adv_examples = []
...
def fgsm_purturb(model, data, target, device, epsilon):
    m = (lambda x: model(x, 0.0)[0]) if isinstance(model, ImplicitRobustLayer) else model
